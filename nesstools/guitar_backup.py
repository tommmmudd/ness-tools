...
import json		# for string material database
...
with open('nesstools/material_dictionary.json') as json_file: 
    try:
    	material_dictionary = json.load(json_file) 
    except:
    	print("couldn't load material_dictionary.json for string materials\nREMOVE THIS WHOLE BIT THEN!")
...
class NESSProject:
    def __init__(self, scriptFile, folder="NESS_projects", projectName="new_project"):
        ...
        self.instName = "inst_"+self.projectName+"_"+str(self.randomNum)+".m"
        self.scoreName = "score_"+self.projectName+"_"+str(self.randomNum)+".m"
        self.tabName = "tab_"+self.projectName+"_"+str(self.randomNum)+".txt"
        self.midiName = "midi_"+self.projectName+"_"+str(self.randomNum)+".mid"
        self.notesName = "notes_"+self.projectName+"_"+str(self.randomNum)+".txt"
        self.textWriter = 0
        self.init()
    ...
    def getName(self):
        return self.projectName+"_"+str(self.randomNum)
    ...
    def write(self):
        ...
        copyfile(os.path.dirname(os.path.realpath(__file__))+"/utils.py", self.directory+"/utils.py")
...
class StringInstrument(object):
    ...
    def frippGuitar(self):
        self.stringCount = 6
        self.defaultGuitar()
        for i in range(self.stringCount):
            self.tuneString(i+1, frippFrets[i])
...
class GuitarScore(object):
    ...
    def palmMuteFromTo(self, strings=1, t=0, endT=1, pos=0.99, glideTime=0.01, fingerF=0.027):
        """palm mute for a specific duratin (require palmmuting flag to be True)"""
        if not isinstance(strings, list):
            strings = [strings]  # make it into a list, and 
        for s in strings:
            s = s-1
            self.palmmuteFingerStrings[s] += str(t-glideTime)+" "+str(pos)+" "+str(fingerF)+"; "
            self.palmmuteFingerStrings[s] += str(t)+" "+str(pos)+" "+str(fingerF)+"; "
            self.palmmuteFingerStrings[s] += str(endT)+" "+str(pos)+" "+str(fingerF)+"; "
    ...
    def vibrato(self, string, t, duration, pos, depth=0.04, rate=5, fingerForce=2):
        ...
        while time < duration:
            phase += phaseDelta
            newPos = pos + (depth * math.sin(phase * 2 * 3.1415927))
            self.playPosition(strings=string, t=t, pos=newPos, glideTime=dt*0.99, fingerForce=fingerForce)
            t += dt
            time += dt

    def movingVibrato(self, string, t, duration, pos, startDepth=0.04, endDepth=0, startRate=5, endRate = 3, fingerForceStart=2, fingerForceEnd=1):
        dt = 0.004
        phase = 0
        #phaseDelta = rate * dt
        time = 0
        while time < duration:
            ...
            force = fingerForceStart + (fraction * (fingerForceEnd - fingerForceStart))
            depth = startDepth + (fraction * (endDepth - startDepth))
            rate = startRate + (fraction * (endRate - startRate))
            phaseDelta = rate * dt
            phase += phaseDelta
            newPos = pos + (depth * math.sin(phase * 2 * 3.1415927))
